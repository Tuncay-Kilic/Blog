...
code = ''' #Importeren functies en vaststellen kleurschema
...
code = ''' # Importeren van de API's : color, sets, themes
...
code = ''' #Hoeveel missende data is er?
...
with col1:
    st.parts_by_year = df_sets[['year', 'num_parts']].groupby('year').mean().reset_index()
...
with col2: 
     ...
     st.parts_year = df_csv_sets[['year', 'num_parts']].groupby('year').mean().reset_index()
